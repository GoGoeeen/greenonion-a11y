-- Create accessibility_scans table
...
CREATE INDEX IF NOT EXISTS idx_accessibility_scans_client_id ON accessibility_scans(client_id);
CREATE INDEX IF NOT EXISTS idx_accessibility_scans_status ON accessibility_scans(status);
...
CREATE POLICY "Enable read access for all users" ON accessibility_scans
    FOR SELECT USING (true); -- Simplified for report viewer without auth

CREATE POLICY "Enable insert for service role only" ON accessibility_scans
...
CREATE POLICY "Enable update for service role" ON accessibility_scans
